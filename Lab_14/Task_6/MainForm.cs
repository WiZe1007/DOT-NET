...
namespace Task_6
{
    public partial class MainForm : Form
    {
        ...
        private void FillObjectFromInputs<T>(T obj)
        {
            ...
            foreach (var prop in properties)
            {
                ...
                if (control is TextBox textBox)
                {
                    ...
                    {
                        prop.SetValue(obj, int.Parse(value)); // Встановити int
                    }
                    else if (prop.PropertyType == typeof(decimal))
                    {
                        prop.SetValue(obj, decimal.Parse(value)); // Встановити decimal
                    }
        ...
        {
            string mode = comboBoxMode.SelectedItem.ToString(); // Отримати режим
            if (mode == "Редагування" && dataGridView.SelectedRows.Count > 0)
            {
